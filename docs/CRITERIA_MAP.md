...
| User interactions | onboarding (crawl → confirm the DNA), document upload + reindex, start a run, **schedule runs on a cadence**, review the output tab by tab, approve/reject with a reason, take the export pack, place posts on a calendar and publish one, brand-voice preferences, platform connections, leads inbox, tool toggles, model routing + sampling, cost. **There is no opportunity-selection screen and no draft editor** — opportunities are ranked inside the run and shown read-only on the review tab, and a draft is approved or rejected rather than edited | `frontend/app/` — **flat**, no route group: `page.tsx` (the PUBLIC front page), `dashboard/`, `automation/`, `content/`, `business/`, `login/`, `onboard/`, `documents/`, `runs/[runId]/`, `leads/`, `memory/`, `connections/`, `developer/{models,runtime,tools,cost}`. The public lead form is not in this tree at all — it is server-rendered by the API at `GET /p/{piece_id}` and posts to `POST /public/forms/{form_id}`. See `ARCHITECTURE.md` §10 | 2–12 |
...
**What "unattended" means here, precisely.** A run can START without a person: an owner sets a cadence on `/automation`, and the scheduler (`backend/app/worker/`, a third process beside the API and the web app) begins the run on its own, checks the business against its monthly spending ceiling first, and pauses the automation with the figures stated rather than spending past it. What it still cannot do is FINISH without a person — the run stops at the review gate, and EXPORT refuses without an approver. So the honest claim is *scheduled, not autonomous*: nothing publishes unattended, by construction rather than by default setting.
