...
| User interactions | onboarding (crawl → confirm the DNA), document upload + reindex, start a run, review the output tab by tab, approve/reject with a reason, take the export pack, brand-voice preferences, platform connections, leads inbox, tool toggles, model routing + sampling, cost. **There is no opportunity-selection screen and no draft editor** — opportunities are ranked inside the run and shown read-only on the review tab, and a draft is approved or rejected rather than edited | `frontend/app/` — **flat**, no route group: `page.tsx`, `login/`, `onboard/`, `documents/`, `runs/[runId]/`, `leads/`, `memory/`, `connections/`, `developer/{models,runtime,tools,cost}`. The public lead form is not in this tree at all — it is server-rendered by the API at `GET /p/{piece_id}` and posts to `POST /public/forms/{form_id}`. See `ARCHITECTURE.md` §10 | 2–12 |
