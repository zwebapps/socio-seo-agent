...
Checked *before* each model call, at three levels: per run (USD + 14 steps), per business per month (USD), per business per week (published pieces — this one is a quality control, not a cost control). Exceeding a cap mid-run ends the run with a partial result and a stated reason. Never an infinite loop.

**The weekly volume cap is the mitigation `ROADMAP.md` §10 promises against scaled content abuse, so it is enforced rather than declared.** `services/publish_cap.weekly_publish_state` is the single decision — a rolling seven-day count of `actions` rows in `PUBLISHABLE_ACTIONS`, against `BUSINESS_WEEKLY_PUBLISH_CAP` (default 10: six channels × one weekly run, plus headroom for manual publishes, and short of two full runs in a week). Both publish paths ask it: EXPORT publishes up to the headroom and names the channels it did not send, following its own per-destination rule rather than discarding approved work wholesale, and the calendar's publish button refuses the one post it was asked about and leaves it queued. `tests/test_run_start_guard.py` fails the build if a third path calls `actuate()` without consulting it.

**The check happens before the idempotency key is claimed, and that ordering is not symmetry with the approval check — it is the opposite of it.** `actuate()` deliberately checks approval *after* the claim so a refusal leaves an auditable row. A volume refusal recorded the same way would poison the key: `claim` returns the outcome a key already holds, so the post could never be published in any later week. A capped publish therefore writes no `actions` row at all — that table is the ledger of *attempted* side effects, and a capped publish attempts nothing. The refusal is reported on the run's `published` report as a `weekly_cap_reached` error, and on the `PublishResult` the button gets back.
